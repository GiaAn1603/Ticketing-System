...
func (h *TicketHandler) InitTicket(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		responses.Error(c, err, "init_ticket", "", h.log)
		return
	}
...
func (h *TicketHandler) BuyTicket(c *gin.Context) {
	...
	*req = requests.BuyRequest{}
	...
	if err := c.ShouldBindJSON(req); err != nil {
		responses.Error(c, err, "buy_ticket", "", h.log)
		return
	}

	reqID := c.GetHeader("X-Request-ID")
	if reqID == "" {
		responses.Error(c, apperrors.ErrMissingRequestID, "buy_ticket", "", h.log)
		return
	}

	if err := h.service.ProcessPurchase(c.Request.Context(), req.EventID, req.UserID, reqID, req.Quantity); err != nil {
		responses.Error(c, err, "buy_ticket", reqID, h.log)
		return
	}
